...
namespace prog_s3 {
    class DataList {
        public:
            ...
            А вообще такое извращение надо затем,
            что сказано "односвязный список _объектов_",
            значит выносить из класса менеджмент памяти (создание нового)
            - моветон и за такое надо бить программиста ногами.

            Поэтому память нужно выделять уже тут, но если я втупую вызову
            конструктор StudentData() - я потеряю те самые свойства,
            которые мы должны показать

            Следовательно, пусть мы и храним поинтеры на некую StudentData,
            Под ними память должна выделяться всё-таки для наследника.
            Тип поинтера в таком случае играет роль подсказки, какой же там
